...
#define displaySCREEN_WIDTH_OFFSET ((DISP_MEM_W-displaySCREEN_WIDTH)/2)
...
void set_x(unsigned col)
{
    ...
    spi(0x10 | (((col+displaySCREEN_WIDTH_OFFSET) >> 4)&0xf));
    mode_data();
}
...
{
	for (unsigned y = 0; y < displaySCREEN_HEIGHT/8; ++y) {
    	set_xrow(0, y);
		for (unsigned x=0; x < displaySCREEN_WIDTH; ++x) {
    		spi(0x00);
    	}
    }
	set_xrow(0,0);
